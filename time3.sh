...
for i in {1..2}
do
	./dinics_prog < test3_graph_input.txt > test3_graph_output.txt

	timing=$(tail -1 test3_graph_output.txt)
	pat='([0-9]+\.[0-9]+)'
	[[ "$timing" =~ $pat ]]
	time=${BASH_REMATCH[0]}
...
for i in {1..10}
do
	./dinics_lct_prog < test3_graph_input.txt > test3_graph_output.txt
